...
export default function DetailsPage() {
  ...
  const categories = useMemo(() => data?.profiledCategoryClientStatementsList ?? [], [data]);

  return (
    <div style={{ padding: 24, maxWidth: 1100, margin: "0 auto" }}>
      <div style={{ marginBottom: 12 }}>
        <Link to="/submissions">← Powrót do listy</Link>
      </div>
        ...
        <>
          ...
          <div style={{ display: "grid", gap: 16 }}>
            {categories.map(cat => {
              ...
              return (
                <div key={cat.category.categoryName} style={{ border: "1px solid #eee", borderRadius: 8, padding: 16 }}>
                  ...
                  <div style={{ marginTop: 8 }}>
                    <div style={{ fontSize: 12, color: "#666" }}>Wspierające: {supPct}%</div>
                    <div style={{ height: 8, background: "#eee", borderRadius: 999, overflow: "hidden" }}>
                      <div style={{ height: "100%", width: `${supPct}%`, background: "#22c55e" }} />
                    </div>
                  </div>

                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 12 }}>
                    <div>
                      <div style={{ fontWeight: 600, marginBottom: 6, color: "#065f46" }}>Wspierające (true)</div>
                    ...
                    <div>
                      <div style={{ fontWeight: 600, marginBottom: 6, color: "#7f1d1d" }}>Ograniczające (true)</div>
                      <ul style={{ margin: 0, paddingLeft: 18 }}>
                        {cat.profiledStatementList
                  ...
                  <details style={{ marginTop: 8 }}>
                    ...
                    <ul style={{ marginTop: 6, paddingLeft: 18 }}>
                      {cat.profiledStatementList
                        ...
                        .map((s, i) => (
                          <li key={`other-${i}`}>
                            <code style={{ marginRight: 6 }}>{s.type === "SUPPORTING" ? "SUPPORT" : "LIMIT"}</code>
                            {s.description}
                          </li>
                        ))}
                    </ul>
